...
void build_image(int number, list_of(struct stbrp_rect) *rects, list_of(TexFileInfo) tex_list) {
	...
	snprintf(file_path, 1024, ".ponygame/sprite_sheet%04d.png", number);
...
void pack_images(const char **tex_paths, size_t path_count) {
	...
	foreach_l(path, tex_paths, path_count, {
	...
	for(;;) {
		...
		build_image(image_index, &rects, tex_list);
		image_index += 1;
